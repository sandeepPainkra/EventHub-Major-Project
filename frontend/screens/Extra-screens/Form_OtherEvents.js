  ...
  const uploadImageAsync = async (uri) => {
    ...
    try {
      const storageRef = ref(
        storage,
        `Images/Event-Category/OtherEvents/Events/image-${Date.now()}`
      );
    ...
    try {
      if (_id) {
        ...
        if (responseData) {
          ...
          setLoading(true);
          navigation.goBack("EventCategory", { loading });
        } else {
          console.log("error");
        }
      }
    } catch (error) {
      console.log("Error in Create Other Event1", error);
    }
  };
...
export default Form_OtherEvents;
