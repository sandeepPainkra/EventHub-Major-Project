...
const EventCategoriesForAvesh = (props) => {
    ...
    fetch(`http://${config.IP_ADDRESS}:5000/api/post/v2/avesh-post/all`, {
      ...
      .then((data) => {
        if (data.status === "ok") {
          ...
          setAyamCategoryData(data.eventAvesh);
        }
      })
      .catch((err) => {
  ...
  // console.log("data is :", AyamCategoryData);
  ...
  return (
    <View className="flex-1 h-full bg-[#1F1F39] px-2">
      ...
      <ScrollView className="h-full">
        ...
        {AyamCategoryData?.map((item, index) => {
          return (
            <TouchableOpacity
              onPress={() => {
...
// for Ayam
const EventCategories_for_Ayam = () => {
  const navigation = useNavigation();
  const [pic, setPic] = useState(null);
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  ...
  const uploadImageAsync = async (uri) => {
    ...
    try {
      const storageRef = ref(
        storage,
        `Images/Event-Category/Ayam/image-${Date.now()}`
      );
      const result = await uploadBytes(storageRef, blob);
      blob.close();
      return await getDownloadURL(storageRef);
    ...
    fetch(`http://${config.IP_ADDRESS}:5000/api/post/v2/ayam-post/all`, {
      ...
      .then((data) => {
        if (data.status === "ok") {
          ...
          setAyamCategoryData(data.eventAyam);
        }
      })
      .catch((err) => {
        console.log("Error in Event category for avesh in frontend is: ", err);
  ...
  console.log("Ayam data is :", AyamCategoryData);
